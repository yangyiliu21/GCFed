...
class Net3(nn.Module):
  def __init__(self, args):
    ...
    self.conv1=nn.Conv2d(3,32,3, padding='same') # in 3 chan, out 32 chan, kernel 3x3
    self.conv2=nn.Conv2d(32,32,3, padding='same') # in 32 chan, out 32 chan, kernel 3x3
    self.conv3=nn.Conv2d(32,64,3, padding='same') # in 32 chan, out 64 chan, kernel 3x3
    self.conv4=nn.Conv2d(64,64,3, padding='same') # in 64 chan, out 64 chan, kernel 3x3
    self.conv5=nn.Conv2d(64,128,3, padding='same') # in 64 chan, out 128 chan, kernel 3x3
    self.conv6=nn.Conv2d(128,128,3, padding='same') # in 64 chan, out 128 chan, kernel 3x3
    self.pool1=nn.MaxPool2d(2,2)
    self.pool2=nn.MaxPool2d(2,2)
    self.pool3=nn.MaxPool2d(2,2)
    self.fc1=nn.Linear(128*8*8,512) # 5x5 comes from  image size
    self.fc2=nn.Linear(512,args.num_classes)

  def forward(self,x):
    x=F.elu(self.conv1(x)) # original image 32x32, out image 30x30  [3, 32, 32] -> [32, 30, 30]       [3, 32, 32] -> [32, 32, 32] 
    x=F.elu(self.conv2(x)) # image 28x28                            [32, 30, 30] -> [32, 28, 28]      [32, 32, 32] -> [32, 32, 32]                                   
    x=self.pool1(x) # image 14x14
    x=F.elu(self.conv3(x)) # image 12x12                            [32, 14, 14] -> [64, 12, 12]      [32, 16, 16] -> [64, 16, 16]       
    x=F.elu(self.conv4(x)) # image 10x10                            [64, 12, 12] -> [64, 10, 10]      [64, 16, 16] -> [64, 16, 16]  
    x=self.pool2(x) # image 5x5
    x=F.elu(self.conv5(x)) #                                                                          [64, 8 8] -> [128, 8, 8]
    x=F.elu(self.conv6(x)) #                                                                          [128, 8 8] -> [128, 8, 8]
    x=x.view(-1,128*8*8) # 5x5 from above                            [64, 5, 5]   -> [, 1600]
    x=F.elu(self.fc1(x)) #                                          [, 1600]     -> [, 512]
    x=self.fc2(x)
    return x
...
class Net2(nn.Module):
  def __init__(self, args):
    ...
    self.fc5=nn.Linear(1024,args.num_classes)
